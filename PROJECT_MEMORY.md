...
- Salto Arianna: `basic-moves/custom_jump.png` misura 3584×3584, griglia 7×7 da 512 px, 49 frame a 30 FPS non ciclici. È un salto neutro senza capriola e senza frame congelati. Lo stacco è indipendente dal valore comune di Mangler tramite `ARIANNA_JUMP_TAKEOFF_FRAME` in `Arianna.gd` (default indice 5, fotogramma visibile 6); l'override di `begin_jump_ascent()` impedisce al callback ereditato di anticiparlo. Usa velocità verticale `character_data.jump_velocity`, gravità Arianna `1400 px/s²` e conserva soltanto la direzione orizzontale scelta a terra. L'ultimo frame non riavvia il salto e il passaggio a idle avviene esclusivamente al contatto col suolo.
