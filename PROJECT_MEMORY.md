...
- Reazioni provvisorie HIGH/LOW Arianna: per impedire che l'ereditarietà da `Mangler.tscn` mostri Mangler quando Player 1 viene colpita, `hurt_high` usa esclusivamente `basic-moves/hurt_high-pose.png` e `hurt_low` usa `basic-moves/hurt_low-pose.png`. Sono animazioni Arianna da un frame a 24 FPS, mantenute visivamente durante l'hitstun; potranno essere sostituite da spritesheet completi senza cambiare i nomi runtime.
...
- Effetto globale `hurt_mid`/`hurt_low`: `Mangler.start_hit_reaction()` genera per qualsiasi fighter un'esplosione additiva azzurra a `(0, -150)` rispetto all'origine del personaggio, quindi all'altezza dello stomaco. L'effetto contiene un lampo radiale e 64 scintille, dura circa mezzo secondo e viene creato una sola volta all'avvio di ciascuna reazione MID o LOW.
