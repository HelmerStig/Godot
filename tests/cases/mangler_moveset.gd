...
static func run(tree: SceneTree, expect: Callable) -> bool:
	...
	expect.call(player1.animated_sprite.animation == &"walk", "avanzando verso destra riproduce walk")
	player1.velocity.x = player1.character_data.run_speed
	player1.change_state(Mangler.State.RUNNING)
	expect.call(player1.animated_sprite.animation == &"run", "RUNNING riproduce run")
	player1.velocity.x = -100.0
	player1.change_state(Mangler.State.WALKING)
	expect.call(player1.animated_sprite.animation == &"backwalk", "arretrando verso sinistra riproduce backwalk")
	player1.is_facing_right = false
	player1.velocity.x = -100.0
	player1.change_state(Mangler.State.WALKING)
	expect.call(player1.animated_sprite.animation == &"walk", "avanzando verso sinistra riproduce walk")
	player1.velocity.x = 100.0
	player1.change_state(Mangler.State.WALKING)
	expect.call(player1.animated_sprite.animation == &"backwalk", "arretrando verso destra riproduce backwalk")
	player1.is_facing_right = true
	player1.velocity = Vector2.ZERO
	player1.change_state(Mangler.State.IDLE)
	expect.call(player1.animated_sprite.animation == &"idle", "IDLE ripristina idle")

	expect.call(player1_bar.value == 100.0, "vita iniziale Player 1 visualizzata")
	expect.call(player2_bar.value == 100.0, "vita iniziale Player 2 visualizzata")
	await tree.create_timer(3.1).timeout
	expect.call(bool(arena.get("round_active")), "training attivo dopo il countdown")
	expect.call(
	...
	await tree.physics_frame
	await tree.process_frame
	expect.call(player1.current_state == Mangler.State.RUNNING, "il doppio tap avanti avvia RUNNING")
	expect.call(
	...
	await tree.physics_frame
	await tree.process_frame
	expect.call(player1.current_state == Mangler.State.RUNNING, "mantenere il secondo avanti continua la corsa")
	Input.action_release(&"p1_move_right")
	await tree.physics_frame
	await tree.process_frame
	expect.call(player1.current_state == Mangler.State.IDLE, "rilasciare avanti termina la corsa")
	...
	player1._on_animation_frame_changed()
	expect.call(player1.current_state == Mangler.State.BACK_HOP, "il frame di stacco avvia BACK_HOP")
	expect.call(
	...
	expect.call(not player1.can_move, "BACK_HOP blocca il controllo fino all'atterraggio")
	expect.call(player1.animated_sprite.animation == &"dodge", "BACK_HOP continua dodge senza riavviarlo")
	player1.velocity = Vector2.ZERO
	player1.change_state(Mangler.State.IDLE)
	...
	await tree.physics_frame
	await tree.process_frame
	expect.call(player1.current_state == Mangler.State.CROUCHING, "tenere giù avvia CROUCHING")
	expect.call(player1.animated_sprite.animation == &"crouch", "CROUCHING riproduce crouch")
	...
	await tree.physics_frame
	await tree.process_frame
	expect.call(player1.current_state == Mangler.State.STANDING_UP, "rilasciare giù avvia STANDING_UP")
	expect.call(player1.animated_sprite.get_playing_speed() < 0.0, "STANDING_UP riproduce crouch al contrario")
	await tree.create_timer(0.75).timeout
	expect.call(player1.current_state == Mangler.State.IDLE, "la rialzata termina in IDLE")
	expect.call(
	...
	await tree.create_timer(player1.get_animation_duration(&"jump_heavy_punch") + 0.05).timeout
	player1.position = aerial_test_position
	player1.velocity = Vector2(0.0, 1.0)
	player1.move_and_slide()
	player1.velocity = Vector2.ZERO
	...
	var sonic_projectiles := tree.get_nodes_in_group("sonic_projectile")
	...
	expect.call(player1.current_state == Mangler.State.ATTACKING, "AttackData avvia lo stato ATTACKING")
	expect.call(
		player1.z_index > player2.z_index,
		"Player 1 passa in primo piano durante qualsiasi attacco"
	)
	expect.call(player1.combat.current_attack == light_punch, "FighterCombat usa la risorsa selezionata")
	...
	await tree.create_timer(light_punch.get_total_duration() + 0.7).timeout
	expect.call(
		player1.current_state == Mangler.State.IDLE,
	...
	await tree.create_timer(player1.get_animation_duration(&"medium_open_hand_slap") + 0.05).timeout
	expect.call(player1.current_state == Mangler.State.IDLE, "lo schiaffo medio completa 1-13 e il ritorno 13-1")
	...
	await tree.create_timer(9.5 / 24.0).timeout
	expect.call(
	...
	await tree.create_timer(player1.get_animation_duration(&"heavy_punch") + 0.05).timeout
	expect.call(player1.current_state == Mangler.State.IDLE, "il pugno pesante completa anche il recupero inverso")
	...
	await tree.create_timer(standing_lk_phases.x + standing_lk_phases.y + standing_lk_phases.z + 0.1).timeout
	expect.call(player1.current_state == Mangler.State.IDLE, "il light kick completa l'intera animazione")
	...
	await tree.create_timer(player1.get_animation_duration(&"crouched_light_kick") + 0.05).timeout
	player2.combat.reset()
	...
	await tree.create_timer(player1.get_animation_duration(&"medium_kick") + 0.05).timeout
	expect.call(player1.current_state == Mangler.State.IDLE, "il medium kick completa l'intera animazione")
	player2.combat.reset()
	...
	await tree.create_timer(player1.get_animation_duration(&"crouched_medium_kick") + 0.05).timeout
	player2.combat.reset()
	...
	await tree.create_timer(player1.get_animation_duration(&"heavy_kick") + 0.05).timeout
	expect.call(player1.current_state == Mangler.State.IDLE, "lo strong kick completa l'animazione")
	player2.combat.reset()
	...
	await tree.create_timer(player1.get_animation_duration(&"crouched_heavy_kick") + 0.05).timeout
	expect.call(
		player1.current_state == Mangler.State.IDLE,
	...
	await tree.create_timer(player1.get_animation_duration(&"crouched_power_punch") + 0.05).timeout
	...
	await tree.create_timer(player1.get_animation_duration(&"crouched_power_punch") + 0.1).timeout
	...
	await tree.create_timer(player1.get_animation_duration(&"crouched_medium_punch") + 0.05).timeout
	player1.change_state(Mangler.State.CROUCHING)
	player1.animated_sprite.pause()
	player1.animated_sprite.frame = 6
	player1.combat.try_attack(&"medium_punch", FighterInputBuffer.Direction.DOWN)
	...
	await tree.create_timer(medium_punch.get_total_duration() + 0.05).timeout
	...
	await tree.create_timer(player1.get_animation_duration(&"crouched_punch") + 0.05).timeout
	player1.change_state(Mangler.State.CROUCHING)
	player1.animated_sprite.frame = 6
	player1.animated_sprite.pause()
	player1.combat.try_attack(&"light_punch", FighterInputBuffer.Direction.DOWN)
	...
	player2.combat.reset()
	player2.change_state(Mangler.State.IDLE)
	await tree.create_timer(player1.get_animation_duration(&"crouched_punch_crouched") + 0.05).timeout
	...
	expect.call(player2.combat.current_health == 80, "danno normale applicato")
	expect.call(player2.current_state == Mangler.State.HIT, "danno normale attiva HIT")
	expect.call(
	...
	await tree.create_timer(0.85).timeout
	expect.call(player2.current_state == Mangler.State.IDLE, "hurt_high completa i fotogrammi 4-16")
	...
	expect.call(player2.current_state == Mangler.State.IDLE, "hurt_low completa i frame 7-16")
	...
	await tree.create_timer(sweep_duration + FighterCombat.SWEEP_GROUNDED_HOLD - 0.1).timeout
	expect.call(
		player2.current_state == Mangler.State.SWEEP_KNOCKDOWN
	...
	await tree.create_timer(1.05).timeout
	expect.call(player2.current_state == Mangler.State.IDLE, "la rialzata completa torna in IDLE")
	...
	expect.call(player2.current_state == Mangler.State.BLOCKING, "guardia attiva BLOCKING")
	expect.call(player2.animated_sprite.animation == &"block_mid", "la guardia centrale riproduce block_mid")
	expect.call(player2_bar.value == 80.0, "la barra non cala durante la guardia")
	await tree.create_timer(player2.get_animation_duration(&"block_mid") + 0.05).timeout
	...
	await tree.create_timer(player2.get_animation_duration(&"block_mid_recovery") + 0.05).timeout
	expect.call(player2.current_state == Mangler.State.IDLE, "block_mid_recovery termina in IDLE")
	...
	await tree.create_timer(player2.get_animation_duration(&"block_high") + 0.05).timeout
	expect.call(
		player2.current_state == Mangler.State.BLOCKING
		and player2.animated_sprite.animation == &"block_high"
	...
	await tree.create_timer(player2.get_animation_duration(&"block_high_recovery") + 0.05).timeout
	expect.call(player2.current_state == Mangler.State.IDLE, "block_high_recovery termina in IDLE")
	...
	expect.call(player2.combat.current_health == 0, "danno letale porta la vita a zero")
	expect.call(player2.current_state == Mangler.State.KNOCKED_DOWN, "danno letale attiva KO")
	expect.call(
	...
	expect.call(not bool(arena.get("round_active")), "KO ferma il training")
	expect.call(not player1.controls_enabled and not player2.controls_enabled, "KO blocca i controlli")
	expect.call(
	...
	expect.call(player1.combat.current_health == player1.combat.max_health, "reset vita Player 1")
	expect.call(player2.combat.current_health == player2.combat.max_health, "reset vita Player 2")
	expect.call(
		player1.current_state == Mangler.State.IDLE,
