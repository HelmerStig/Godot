...
- Il salto neutro di Arianna usa tutti i 49 frame della griglia 7×7 di `basic-moves/custom_jump.png` a 30 FPS, senza loop e senza fotogrammi congelati. Lo stacco è configurabile indipendentemente tramite `ARIANNA_JUMP_TAKEOFF_FRAME` in `scripts/Arianna.gd`: il valore è zero-based, quindi `5` corrisponde al fotogramma visibile 6. Torna in idle esclusivamente al contatto col suolo; eventuali rotazioni restano riservate alle mosse aeree dedicate.
